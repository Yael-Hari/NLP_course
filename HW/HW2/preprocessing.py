...
class SentencesEmbeddingDataset:
    ...
    def get_data_loaders(self, batch_size):
        X_train, y_train, sentences_lengths_train, X_dev, y_dev, sentences_lengths_dev = self.get_datasets()
    ...
    def get_X_test(self):
        X_test, _, sentences_lengths_test = self._get_dataset_from_path(self.test_path, tagged=False)
        return X_test, sentences_lengths_test

    def get_datasets(self):
        X_train, y_train, sentences_lengths_train = self._get_dataset_from_path(self.train_path, tagged=True)
        X_dev, y_dev, sentences_lengths_dev = self._get_dataset_from_path(self.dev_path, tagged=True)
        return X_train, y_train, sentences_lengths_train, X_dev, y_dev, sentences_lengths_dev
    ...
    def _get_dataset_from_path(self, path: str, tagged: bool):
        ...
        if tagged:
            # make labels binary
            y = [torch.Tensor([0 if y_ == 'O' else 1 for y_ in sentence_tags]) for sentence_tags in y]
